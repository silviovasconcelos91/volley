...
class PlayerRepositoryAdapterTest {
    ...
    fun should_findPlayer_when_existingId() {
        val saved = playerRepository.save(
            Player(name = "Jane", surname = "Doe", email = "jane@test.com", age = 30, level = Level.ADVANCED)
        )
    ...
    fun should_returnAllPlayers_when_playersExist() {
        playerRepository.save(Player(name = "Player1", surname = "Test", email = "p1@test.com", age = 20, level = Level.BEGINNER))
        playerRepository.save(Player(name = "Player2", surname = "Test", email = "p2@test.com", age = 25, level = Level.EXPERT))
    ...
    fun should_deletePlayer_when_existingId() {
        val saved = playerRepository.save(
            Player(name = "ToDelete", surname = "Test", email = "del@test.com", age = 22, level = Level.INTERMEDIATE)
        )
